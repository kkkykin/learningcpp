...
#+end_src
** Virtual destructors, virtual assignment, and overriding virtualizatio
...
We can change entire class definition or one member function!
#+begin_src cpp
#include <iostream>
...
Partially specialize a function, which is disallowed, Partially specialize the entire class instead. Using a common base class significantly less duplicated code.
#+begin_src cpp :main no
#include <iostream>
...
In debug builds, it can be useful to disable the catch-all handler.
#+begin_src cpp :main no
#include <iostream>
...
*** Cleaning up resources after try-catch
#+begin_src cpp
#include <iostream>
...
:PROPERTIES:
:header-args:cpp: :results output replace :includes <iostream>
:END:
*** Formatting
...
setf() only turns flags on -- it isn’t smart enough to turn mutually exclusive flags off. Consequently, when we turned std::hex on, std::ios::dec was still on, and std::ios::dec apparently takes precedence.
#+begin_src cpp
...
std::cout << os.str();          // Use the str() function to retrieve the results of the buffer:
...
string to number
#+begin_src cpp :includes '(<iostream> <string>)
std::stringstream os {};
...
os.clear(); // reset error flags, recommend
